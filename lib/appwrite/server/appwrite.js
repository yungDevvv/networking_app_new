...
export async function createRecoveryPassword(redirectEmail) {
...
export async function getVisibleProfiles() {
  ...
  try {
    ...
    // if (currentUser.networks && currentUser.networks.length > 0) {
    //   const networksQuery = currentUser.networks.map(network =>
    //     Query.equal('$id', network.$id)
    //   );

    //   const networks = await database.listDocuments(
    //     "main_db",
    //     'networks',
    //     [Query.or(networksQuery)]
    //   );

    //   networkMembers = networks.documents;
    // }
